  ...
  return (
    <Card className="space-y-3">
      <div className="flex items-start justify-between">
        ...
        <button onClick={() => onDelete(goal.id)} className="p-1 text-muted hover:text-destructive">
          <Trash2 size={14} />
        </button>
      </div>
        ...
        <div className="relative w-16 h-16 shrink-0">
          <svg viewBox="0 0 36 36" className="w-full h-full -rotate-90">
            ...
            <circle
              ...
              stroke={goal.is_completed ? 'var(--color-income)' : 'var(--color-accent)'}
              strokeWidth="3"
  ...
  return (
    <PageTransition>
      <div className="p-4 space-y-4">
        <div className="flex items-center justify-between">
          ...
          {!showForm && (
            <Button size="sm" onClick={() => setShowForm(true)}>
              <Plus size={16} className="mr-1" /> {t('savings.create_new')}
            </Button>
          )}
        </div>
        ...
        {showForm && (
          <Card>
            <div className="space-y-3">
              ...
              <div>
                <label className="block text-xs text-muted mb-1">{t('savings.target')}</label>
                <input
                  inputMode="decimal"
                  placeholder="0.00"
                  value={targetStr}
              ...
              <div>
                <label className="block text-xs text-muted mb-1">{t('savings.deadline')} ({t('transactions.note_placeholder').split('(')[1]?.replace(')', '') || 'optional'})</label>
                <input
                  type="date"
                  value={deadline}
        ...
        {/* Deposit/Withdraw amount overlay */}
        {showAmountFor && (
          <Card>
            <div className="space-y-3">
